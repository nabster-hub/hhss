    ...
    return (
        <section id={blok.anchorLink}>
            <div className={"container py-24"}>
                <h2 className={"block w-[76%] mx-auto lg:w-full font-primary text-center lg:text-left font-medium text-[28px] lg:text-[32px] leading-10 mb-9"}>{blok.title}</h2>
                    ...
                    {blok.column.map((column, _uid) => (
                        <div key={_uid} className={"flex flex-col gap-7 p-7 bg-white rounded-lg"}>
                            <div className="">
                                ...
                                <h3 className={"font-primary font-medium text-2xl"}>{column.title}</h3>
                            </div>
                            <hr/>
